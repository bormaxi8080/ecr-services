...
//
// mmx, 2008
// Сервис извлечения картинок из xml (формат base64) в бинарный вид.
// Работает по заданным папкам
//
// *********************************************************************************************************************************
...
{
	/// <summary>
	/// Оберточный класс, реализующий функциональность класса FilesExtractorAgent в консоли
	/// </summary>
	class FilesExtractorManager
	{
...
#pragma warning restore 169
		...
		{
			var _assembly_name = _assembly.GetName().Name;
			_log.Info("Process started");
			_log_notifications.Info(string.Format("Start notification created for process: {0}", _assembly_name));
			try
			{
				...
				DEBUG = Convert.ToBoolean(_extender.GetValue(string.Format("{0}.ApplicationDebugMode", _assembly_name), "false"));
				PrintDebugHeader();

				var _agent = new FilesExtractorAgent(DEBUG);
				_agent.Execute();
			...
			catch (Exception e)
			{
				...
				_log_notifications.Fatal(string.Format("Error notification created for process: {0}", _assembly_name), e);
			}
			finally
			{
				_log_notifications.Info(string.Format("End notification created for process: {0}", _assembly_name));
			}
		}
