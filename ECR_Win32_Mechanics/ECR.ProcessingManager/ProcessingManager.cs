...
//
// mmx, 2010.
// Сервис загрузки файлов бинарного контента в базы данных хранилища ECR.
//
// *********************************************************************************************************************************
...
#pragma warning restore 169
        ...
        private static void Execute(string[] p_args)
        {
            ...
            _log.Info("Запуск приложения");
            _log_notifications.Info(string.Format("Start notification message created for process: {0}", _assemblyName));
            try
            {
                ...
                var _agent = new ProcessingAgent(DEBUG);
                _agent.Execute();
            ...
            catch (Exception e)
            {
                ...
                _log_notifications.Fatal(string.Format("Error notification message created for process: {0}", _assemblyName), e);
            }
            finally
            {
                _log_notifications.Info(string.Format("End notification created for process end: {0}", _assemblyName));
            }
        }
