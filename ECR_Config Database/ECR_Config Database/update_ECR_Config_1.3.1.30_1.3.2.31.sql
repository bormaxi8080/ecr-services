...
select
    @OldVersion = N'1.3.1.30'
    , @NewVersion = N'1.3.2.31'
    , @Description = left(N'Добавление колонки Params в таблицы ecr.Entities, ecr.Views', 255);
...
GO
ALTER TABLE ecr.Entities ADD Params nvarchar(255) null;
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
...
ALTER TABLE ecr.Views ADD Params nvarchar(255) null;
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
