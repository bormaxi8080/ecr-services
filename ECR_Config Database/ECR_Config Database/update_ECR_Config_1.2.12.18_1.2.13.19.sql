...
select
    @OldVersion = N'1.2.12.18'
    , @NewVersion = N'1.2.13.19'
    , @Description = left(N'Патч хранимок отчета по файловому контенту', 255);
...
GO
PRINT N'Creating procedure [ecr].[FileContentReportByRegion_Goods_Everyday]'
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
...
PRINT N'Altering procedure [ecr].[FileContentReportByRegion_Goods]'
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
