...
select
    @OldVersion = N'1.2.2.8'
    , @NewVersion = N'1.2.3.9'
    , @Description = left(N'Добавлены/изменены объекты процессинга входящих файлов', 255);
...
GO
/****** Object:  StoredProcedure [dbo].[MarkProcessedPackage]    Script Date: 05/26/2009 19:11:04 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MarkProcessedPackage]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MarkProcessedPackage]
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
...
/****** Object:  StoredProcedure [dbo].[MarkProcessedPackage]    Script Date: 05/26/2009 19:11:21 ******/
SET ANSI_NULLS ON
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
...
-- =============================================
CREATE PROCEDURE [ecr].[MarkProcessedPackage] 
...
/****** Object:  StoredProcedure [ecr].[AddProcessedPackage]    Script Date: 05/26/2009 19:11:42 ******/
SET ANSI_NULLS ON
GO
IF @@ERROR<>0 AND @@TRANCOUNT>0 ROLLBACK TRANSACTION
GO
...
-- Author:		MMX
-- Create date: 26.05.2009
-- Description:	Процедура добавляет запись в таблицу ecr.ProxcessedPackages
-- =============================================
CREATE PROCEDURE [ecr].[AddProcessedPackage] 
	-- Add the parameters for the stored procedure here
...
BEGIN
    ...
    -- Insert statements for procedure here
	insert into [ecr].ProcessedPackages
		(
		PackageID,
		DateCreate,
		DateProcessed
